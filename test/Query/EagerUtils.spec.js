...
import { eagerNode, eagerPattern, } from '../../src/Query/EagerUtils';
...
describe('Query/EagerUtils.js', () => {
    ...
    describe('eagerPattern', () => {
        ...
        it('should build a pattern for `relationship` and append [0]', () => {
            ...
            const output = eagerPattern(instance, 1, 'this', rel).replace(/\n/g, '').replace(/\s\s/g, '');
            const expected = 'directorRel: [ (this)<-[this_directorRel_rel:`DIRECTED`]-(this_directorRel_node) |this_directorRel_rel { .*,__EAGER_ID__: id(this_directorRel_rel),__EAGER_TYPE__: type(this_directorRel_rel),director:this_directorRel_node { .*,__EAGER_ID__: id(this_directorRel_node),__EAGER_LABELS__: labels(this_directorRel_node)}} ][0]';
    ...
    describe('eagerNode', () => {
        ...
        it('should request a nested eager statement', () => {
            const props = 'movies: [ (this_directorNode_node)-[this_directorNode_node_movies_rel:`ACTED_IN`]->(this_directorNode_node_movies_node) |this_directorNode_node_movies_node {.*,__EAGER_ID__: id(this_directorNode_node_movies_node),__EAGER_LABELS__: labels(this_directorNode_node_movies_node)} ]}';
            expect(pattern.indexOf(props) > -1).to.equal(true);
        });
        ...
        it('should request an eager `relationship`', () => {
            const props = 'directorRel: [ (this)<-[this_directorRel_rel:`DIRECTED`]-(this_directorRel_node) |this_directorRel_rel {.*,__EAGER_ID__: id(this_directorRel_rel),__EAGER_TYPE__: type(this_directorRel_rel)';
            const director_props = 'director:this_directorRel_node {.*,__EAGER_ID__: id(this_directorRel_node),__EAGER_LABELS__: labels(this_directorRel_node)}} ][0]';
