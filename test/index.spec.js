...
import { Driver } from 'neo4j-driver/lib/driver';
...
describe('index.js', () => {
            ...
            Promise.all([
                ...
                .then(([from, to]) => {
                    return instance.relate(from, to, 'unknown')
                        .then(() => assert(false, 'Error should be thrown on unknown relationship type'))
                        .catch(() => done());
                });
        });
    });
    ...
    describe('::cypher', () => {
        it('should execute a function as part of a session', (done) => {
            ...
            instance.cypher('MATCH (n) RETURN COUNT(n)', {}, session)
                .then(() => expect(session._open).to.equal(false))
                .then(() => done())
                .catch(e => done(e));
        });
    });
    ...
    describe('::close', () => {
        it('should close the neo4j connection', (done) => {
            const instance = require('./instance')();
            instance.close().then(() => done());
        });
    });
});